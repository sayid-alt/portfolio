...
def forecast_chart(df, title_chart):
    ...
    with metric_col1:
        ...
        with lstm:
            st.metric(label="LSTM", value=round(
                lstm_metrics['rmse'], 4), border=False)

        with gru:
            st.metric(label="GRU", value=round(
                gru_metrics['rmse'], 4), border=False)
    ...
    with metric_col2:
        ...
        with lstm:
            st.metric(label="LSTM", value=round(
                lstm_metrics['mae'], 4), border=False)
        with gru:
            st.metric(label="GRU", value=round(
                gru_metrics['mae'], 4), border=False)
    ...
    with metric_col3:
        ...
        with lstm:
            st.metric(
                label="LSTM", value=f"{round(lstm_metrics['mape'], 3)}%", border=False)
        with gru:
            st.metric(
                label="GRU", value=f"{round(gru_metrics['mape'], 3)}%", border=False)
